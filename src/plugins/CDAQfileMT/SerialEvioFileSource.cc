 ...
 * Constructor
 */
SerialEvioFileSource::SerialEvioFileSource(std::string resource_name, JApplication *app) : JEventSource(resource_name,
                                                                                                    app) {
 ...
 * Destructor
 */
SerialEvioFileSource::~SerialEvioFileSource() {
    if (m_hdevio) {
        m_hdevio->PrintStats();
        m_hdevio.reset();
    }
...
void SerialEvioFileSource::OpenEVIOFile(std::string filename) {
 ...
 */
void SerialEvioFileSource::GetEvent(std::shared_ptr <JEvent> event) {
...
std::string SerialEvioFileSource::GetDescription() {
    return "SerialEvioFileSource - Read an *.evio file and parse the blocks into JEvents.";
}


template<>
double JEventSourceGeneratorT<SerialEvioFileSource>::CheckOpenable(std::string resource_name) {
